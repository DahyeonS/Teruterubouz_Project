        ...
        function imageLoad(num) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let filelist = '';
                    if (fileName !== '') {
                        filelist = '업로드된 이미지<br>'
                        for (let i=0; i<fileId.split(', ').length; i++) {
                            const newFileId = encodeURIComponent(fileId.split(', ')[i]);
                            filelist += '<img id="image' + i + '" src="../resources/uploads/s_' + newFileId + '" style="width: 200px; height: 150px" alt="' + fileName.split(',')[i] + '"/>&nbsp&nbsp'
                            + '<a href="#" onclick="imgDeleteConfirm(' + num + ', \'' + newFileId + '\', \'' + fileName.split(',')[i] + '\');">X</a>&nbsp&nbsp'
                        }
                        $('#filelist').show();
                    }
